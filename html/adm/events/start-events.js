firebase.auth().onAuthStateChanged((User) => {
  if (!User) {
      ...
      const ativar_activity = document.getElementById("start-events-form");
      ...
      document.getElementById("start-events-form").addEventListener("submit", function(event) {
        ...
        const activity_date = new Date((document.getElementById("event_date").value).replace('-','/')).toLocaleDateString('pt-BR');
        const activity_id = document.getElementById("event_id").value;
        const activity_state = "waiting"; // "waiting", "started", "finished"

        let linhas = ''; 
        eventService.eventsbyDateStart(event_id, event_date, event_state).then(events => {
